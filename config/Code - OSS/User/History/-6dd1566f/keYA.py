...
def process_stats(val, name,rollno):
    if not val : val = {'Easy': 0,'Medium': 0,'Hard' : 0}
    return pd.DataFrame(val,index=[rollno+"-"+name])
...
def fetch_stats(date):
    ...
    if os.path.isfile(OUTPUT_PATH):
        ...
        # print(date in df1.columns[0])
        # df1=df1.drop([df2.columns],axis=1)
        print(df1.columns.drop([date.strftime]),sep='\n')
        # else:
